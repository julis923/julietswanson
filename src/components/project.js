...
const Project = ({
  ...
  const handleProjectView = () => {
    ...
    if (viewDetails) {
      projectWrapper.current.scrollIntoView({
        behavior: 'smooth',
        block: 'start',
      });
    } else {
      projectDetails.current.scrollIntoView({
        behavior: 'smooth',
        block: 'start',
      });
    }
  ...
  return (
    <>
          ...
          >
            <p
              className={`details-toggle ${
                toggler ? 'details-toggle-focus' : ''
              } ${viewDetails ? 'details-toggle-view' : ''}`}
            >
              {viewDetails ? 'Hide' : 'Show'} Full Details
            </p>
            <ChevronSvg
              fill={toggler || viewDetails ? '#FFBD13' : '#3f3e3f'}
      ...
      <div
        className={`${viewDetails ? 'project-details' : 'hidden-details'} ${
                ...
                {projectProcess.map((item, i) => {
                  return (
                    <div className="list-section" key={`${title}-process-${i}`}>
                      ...
                      <div
                        className={item.images[0] ? 'list-section-images' : ''}
                      >
